...
export default function Header() {
  ...
  return (
    <header className="fixed top-0 left-0 w-full border-b bg-white z-[1000]">
      <div className="flex items-center justify-between px-4 md:px-8 py-3 gap-6">
        ...
        <nav className="hidden md:flex items-center gap-6 text-base font-semibold">
          ...
          >
            <button className="flex items-center gap-1">Ressources ▾</button>
            {resOpen && (
              <div className="absolute top-full left-0 pt-2 min-w-[200px] z-20">
                <div className="bg-white border shadow-md rounded">
                  {resources.map((r) => (
                    <Link key={r.href} href={r.href} className="block px-4 py-2 hover:bg-gray-50">
                      {r.label}
                    </Link>
                  ))}
                </div>
